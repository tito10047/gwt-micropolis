...
*/
	boolean loadMap(String mapStr)
	{
		char[] chars = mapStr.toCharArray();
		if (chars.length!=DEFAULT_HEIGHT*DEFAULT_WIDTH){
			return false;
		}
		int pos = 0;
		char[][] newMap = new char[DEFAULT_HEIGHT][DEFAULT_WIDTH];
		for (int x = 0; x < DEFAULT_WIDTH; x++)
		{
			for (int y = 0; y < DEFAULT_HEIGHT; y++)
			{
				newMap[y][x] = chars[pos++];
			}
		}
		map = newMap;
		return true;
	...
	String writeMap()
	{
		char[] dis = new char[DEFAULT_HEIGHT*DEFAULT_WIDTH];
		int i = 0;
		for (int x = 0; x < DEFAULT_WIDTH; x++)
		{
			for (int y = 0; y < DEFAULT_HEIGHT; y++)
...
/*
	...
	public void save(Map map){
		map.setHistoryRes(history.res);
		map.setHistoryCom(history.com);
		map.setHistoryInd(history.ind);
		map.setHistoryCrime(history.crime);
		map.setHistoryPollution(history.pollution);
		map.setHistoryMoney(history.money);
		map.setMap(writeMap());

		map.setResPop((short) resPop);
		map.setComPop((short) comPop);
		map.setIndPop((short) indPop);
		map.setResValve((short) resValve);
		map.setComValve((short) comValve);
		map.setIndValve((short) indValve);
		//8
		map.setCityTime(cityTime);
		map.setCrimeRamp((short) crimeRamp);
		map.setPolluteRamp((short) polluteRamp);
		//12
		map.setLandValueAverage((short) landValueAverage);
		map.setCrimeAverage((short) crimeAverage);
		map.setPollutionAverage((short) pollutionAverage);
		map.setGameLevel((short) gameLevel);
		//16
		map.setEvaluationCityClass((short) evaluation.cityClass);
		map.setEvaluationCityScore((short) evaluation.cityScore);
		//18
		//50
		map.setBudgetTotalFunds(budget.totalFunds);
		map.setAutoBulldoze(autoBulldoze);
		map.setAutoBudget(autoBudget);
		//54
	...
	public boolean load(Map map){
		if (loadMap(map.getMap())==false){
			return false;
		}
		history.res = map.getHistoryRes();
		history.com = map.getHistoryCom();
		history.ind = map.getHistoryInd();
		history.crime = map.getHistoryCrime();
		history.pollution = map.getHistoryPollution();
		history.money = map.getHistoryMoney();

		resPop = map.getResPop();
		comPop = map.getComPop();
		indPop = map.getIndPop();
		resValve = map.getResValve();
		comValve = map.getComValve();
		indValve = map.getIndValve();
		//8
		cityTime = map.getCityTime();
		crimeRamp = map.getCrimeRamp();
		polluteRamp = map.getPolluteRamp();
		//12
		landValueAverage = map.getLandValueAverage();
		crimeAverage = map.getCrimeAverage();
		pollutionAverage = map.getPollutionAverage();
		gameLevel = map.getGameLevel();
		//16
		evaluation.cityClass = map.getEvaluationCityClass();
		evaluation.cityScore = map.getEvaluationCityScore();
		//18
		//50
		budget.totalFunds = map.getBudgetTotalFunds();
		autoBulldoze = map.isAutoBulldoze();
		autoBudget = map.isAutoBudget();
		//54
		autoGo = map.isAutoGo();
		//userSoundOn
