...
public class GraphsPane implements Micropolis.Listener {
	Micropolis engine;

	GraphArea graphArea;

	static enum TimePeriod {
		TEN_YEARS, ONETWENTY_YEARS;
	}

	static enum GraphData {
		RESPOP, COMPOP, INDPOP, MONEY, CRIME, POLLUTION;
	}
	...
	public GraphsPane(Micropolis engine) {
		...
		c.gridy++;
		onetwentyYearsBtn = new JToggleButton(
				strings.getString("onetwenty_years"));
	...
	private void makeDataBtn(GraphData graph) {
		...
		String icon2name = strings.getString("graph_button." + graph.name()
				+ ".selected");
	...
	class GraphArea {
		...
		public void paintComponent() {
			...
			int bottomEdge = getHeight() - getInsets().bottom - getInsets().top
					- BOTTOM_MARGIN;
			int rightEdge = getWidth() - getInsets().right - getInsets().left
					- RIGHT_MARGIN - maxLabelWidth - LEGEND_PADDING;
			...
			int unitPeriod = isOneTwenty ? 12 * Micropolis.CENSUSRATE
					: Micropolis.CENSUSRATE;
			int hashPeriod = isOneTwenty ? 10 * unitPeriod : 12 * unitPeriod;
			int startTime = ((engine.history.cityTime / unitPeriod) - 119)
					* unitPeriod;
			...
			for (GraphData gd : GraphData.values()) {
				if (dataBtns.get(gd).isSelected()) {
					...
					for (int i = 0; i < 120; i++) {
						...
						double yp = bottomEdge - getHistoryValue(gd, H - i)
								* (bottomEdge - topEdge) / 256.0;
						if (i == 0) {
							path.moveTo(xp, yp);
						} else {
							path.lineTo(xp, yp);
			...
			for (GraphData gd : myGraphs) {
				...
				int y = (int) Math.round(path.getCurrentPoint().getY()
						+ fm.getAscent() / 2);
